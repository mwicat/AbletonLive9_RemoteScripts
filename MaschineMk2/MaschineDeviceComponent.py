...
# File: MaschineDeviceComponent.pyc (Python 2.5)
...
class MaschineDeviceComponent(DeviceComponent):
    ...
    def update(self):
        if self.is_enabled() and self._device != None:
            ...
            if self._bank_up_button != None and self._bank_down_button != None:
                if not self._bank_index == None:
                    pass
                can_bank_up = self._number_of_parameter_banks() > self._bank_index + 1
                if not self._bank_index == None:
                    pass
                can_bank_down = self._bank_index > 0
    ...
    def enter_clear_mode(self):
        ...
        for control in self._parameter_controls:
            ...
            if not self.touch_mode:
                pass
            control.add_value_listener(self._clear_param, True)
    ...
    def exit_clear_mode(self):
        ...
        for control in self._parameter_controls:
            if not self.touch_mode:
                pass
            control.remove_value_listener(self._clear_param)
    ...
    def _clear_param(self, value, control):
        ...
        if self._device and key in self.del_parm_map:
            if not key in self.del_clip_map or self.del_clip_map[key]:
                pass
            prev_clip = None
