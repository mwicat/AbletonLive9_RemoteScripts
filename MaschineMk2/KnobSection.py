...
# File: KnobSection.pyc (Python 2.5)
...
class KnobSection(CompoundComponent):
    ...
    def __init__(self, modeselector, editsection, *a, **k):
        super(KnobSection, self).__init__(*a, **a)
        ...
        self._radio_buttons = (self.volume_button, self.swing_button, self.tempo_button, self.xfade_button, self._color_edit_button)
    ...
    def set_scrub_mode(self, value):
        ...
        if not value or 127:
            pass
        self.scrub_mode_button.send_value(0)
    ...
    def _do_channel_slider(self, value):
        ...
        if self._editsection.isShiftdown():
            if not value == 127 or Live.Application.Application.View.NavDirection.left:
                pass
            dir = Live.Application.Application.View.NavDirection.right
        ...
        elif not value == 127 or Live.Application.Application.View.NavDirection.left:
            pass
    ...
    def do_main(self, value):
        if self._wheel_overide:
            if not value == 1 or 1:
                pass
            self._wheel_overide(-1, self._editsection.isShiftdown(), self._push_down)
        elif self.knob_action:
            self.knob_action(value)
        

    do_main = subject_slot('value')(do_main)
    ...
    _do_toggle_mode = subject_slot('value')(_do_toggle_mode)
    ...
    do_main_push = subject_slot('value')(do_main_push)
    ...
    _do_color_button = subject_slot('value')(_do_color_button)
    ...
    _do_button_left = subject_slot('value')(_do_button_left)
    ...
    _do_button_right = subject_slot('value')(_do_button_right)
    ...
    _do_mikr_cliplen = subject_slot('value')(_do_mikr_cliplen)
    ...
    _do_mikr_quantize = subject_slot('value')(_do_mikr_quantize)
    
    def _modify_init_cliplen(self, value):
        if self._push_down:
            if not value == 1 or 1:
                pass
            self._editsection.mod_new_initlen(-1)
        elif not value == 1 or 4:
            pass
        self._editsection.mod_new_initlen(-4)

    
    def _modify_quant_grid(self, value):
        if not value == 1 or 1:
            pass
        self._editsection.mod_quant_size(-1)
    ...
    _do_xfade = subject_slot('value')(_do_xfade)
    ...
    _do_volume = subject_slot('value')(_do_volume)
    ...
    _do_swing = subject_slot('value')(_do_swing)
    ...
    _do_tempo = subject_slot('value')(_do_tempo)
    ...
    _do_dedicated_clip_quantize = subject_slot('value')(_do_dedicated_clip_quantize)
    ...
    _do_dedicated_rec_quantize = subject_slot('value')(_do_dedicated_rec_quantize)
