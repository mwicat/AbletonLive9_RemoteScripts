...
class SceneElement:
    ...
    def set_button(self, button, scene, index):
        if not button == None and isinstance(button, ButtonElement):
            raise AssertionError
        if not scene == None and isinstance(scene, Live.Scene.Scene):
            raise AssertionError
        self._scene = scene
        self._index = index
...
class SceneMode(MaschineMode):
    ...
    def __init__(self, button_index, *a, **k):
        super(SceneMode, self).__init__(button_index, *a, **a)
        self.elements = (tuple,)(lambda .0: for idx in .0:
SceneElement(idx, self)(range(16)))
        self.offset = 0
